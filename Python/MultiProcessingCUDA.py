...
tracker = cv2.cuda_TrackerCSRT_create()
...
def track_object(video_path,output):
    ...
    bbox = x, y, w, h
    tracker.init(cv2.cuda_GpuMat(frame), bbox)
    # Khởi tạo CSRT Tracker
    while True:
        # Đặt vị trí ban đầu của đối tượng cần tracking
        success, frame = cap.read()
        # Chuyển đổi frame sang dạng GpuMat
...
if __name__ == '__main__':
    ...
    while True:
        ...
        for (center1, frame1), (center2, frame2) in zip(output1, output2):
            ...
            # print("center2: ", center2)
            depth = tri.find_depth(center2, center1, frame2, frame1, B, f, alpha)
            print("depth: ", depth)
